...
TAG_MAP = {
    ...
    "waste": "Disposal",
    "tip": "Disposal",
    "clearance": "Rules & Safety",
    "monitor": "Rules & Safety",
    "friable": "Rules & Safety",
    "myself": "Rules & Safety",
    "diy": "Rules & Safety",
    "soil": "Testing",
    "cost": "Costs",
...
def guess_tag(title, keyword):
    ...
    for fragment, tag in TAG_MAP.items():
        if fragment in text:
            return tag
    return "Asbestos Guides"


def make_nav_title(title):
    nav = re.sub(r'\s*\|.*$', '', title)
    nav = re.sub(r'\s*[-–—]\s*Barwon Asbestos Co.*$', '', nav)
    nav = re.sub(r'\s*\(20\d{2}.*?\)', '', nav)
    nav = re.sub(r"\s*Geelong:?\s*", ' ', nav).strip()
    if len(nav) <= 30:
        return nav
    words = nav.split()
    result = ''
...
def clean_body_links(body):
    body = re.sub(r'https?://barwonasbestos\.com/', '/', body)
...
def convert_and_write(draft_path, output_path, publish_date, slug):
    ...
    canonical = f'https://barwonasbestos.com/blog/{slug}/'
